...
# initial data
cards = {
    0: {"name": "Vexscream", "strength": 1, "speed": 6, "stealth": 21,
        "cunning": 19, "lock": True, "image": "vexscream"},
    1: {"name": "Dawnmirage", "strength": 5, "speed": 15, "stealth": 18,
        "cunning": 22, "lock": True, "image": "dawnmirage"},
    2: {"name": "Blazegolem", "strength": 15, "speed": 20, "stealth": 23,
        "cunning": 6, "lock": True, "image": "blazegolem"},
    3: {"name": "Moldvine", "strength": 21, "speed": 18, "stealth": 14,
        "cunning": 5, "lock": True, "image": "moldvine"},
    4: {"name": "Vortexwing", "strength": 19, "speed": 13, "stealth": 19,
        "cunning": 2, "lock": True, "image": "vortexwing"},
    5: {"name": "Froststep", "strength": 14, "speed": 14, "stealth": 17,
        "cunning": 4, "lock": True, "image": "froststep"},
    6: {"name": "Wispghoul", "strength": 17, "speed": 19, "stealth": 3,
        "cunning": 2, "lock": True, "image": "wispghoul"}
}

# The states and current state of the sort button
...
# Sets the idx to the button pressed
...
def addbutton():
    ...
    cards[idx] = {"name": "", "strength": 1, "speed": 1, "stealth": 1,
                  "cunning": 1, "total score": 4}
...
def cardview():
    ...
    totalviewoutput.config(
        text=(cards[idx]["cunning"] + cards[idx]["stealth"] +
              cards[idx]["speed"] + cards[idx]["strength"])
    )
...
def sortpress():
    global sortcurrent
    for idx in cards:
        cards[idx]["total score"] = (
            cards[idx]["cunning"] + cards[idx]["stealth"] +
            cards[idx]["speed"] + cards[idx]["strength"]
        )
    if sortcurrent == 0:
        sortcurrent += 1
...
def printbutton():
    ...
    for id in cards:
        temp += f"Name: {str(cards[id]['name'])} Strength: {str(cards[id]['strength'])} Speed: {str(cards[id]['speed'])} Stealth: {str(cards[id]['stealth'])} Cunning: {str(cards[id]['cunning'])} Total Score: {str(cards[id]['cunning'] + cards[id]['stealth'] + cards[id]['speed'] + cards[id]['strength'])} \n"
        temp += "-" * 120 + "\n"
...
title = tk.Label(
    mainframe, text="Monster Card Catalogue", bg="#1f57a2",
    font=("Anton", 30, "bold")
)
title.grid(row=0, column=0, columnspan=5)

sortbutton = tk.Button(
    mainframe, text="sort", width=10, height=1, command=sortpress
)
sortbutton.grid(row=1, column=0)

searchbar = tk.Entry(
    mainframe, width=12, font=("Anton", 12, "bold")
)
searchbar.grid(row=1, column=3)

searchbutton = tk.Button(
    mainframe, width=10, font=("Anton", 8, "bold"),
    text="Search", command=searchpress
)
...
for row in range(2):
    for col in range(5):
        if i in cards:
            ...
            hold = "empty"
        hold = tk.Button(
            mainframe, text=hold, width=88, height=80,
            bg="#a6a6a6", command=lambda idx=i: buttonmonsters(idx),
            relief='flat', compound="top", font=("Anton", 8, "bold")
        )
        if i in cards:
...
addcard = tk.Button(
    main2frame, text="Add card", width=10, height=2, relief='flat',
    font=("Anton", 8, "bold"), command=addbutton
)
addcard.grid(row=0, column=0, padx=20, pady=10)
removecard = tk.Button(
    main2frame, text="Remove card", width=10, height=2, relief='flat',
    font=("Anton", 8, "bold"), command=deletecard
)
removecard.grid(row=0, column=1, padx=14, pady=10)
editcard = tk.Button(
    main2frame, text="Edit card", width=10, height=2, relief='flat',
    font=("Anton", 8, "bold"), command=editbutton
)
editcard.grid(row=0, column=2, padx=14, pady=10)
viewcard = tk.Button(
    main2frame, text="View card", width=10, height=2, relief='flat',
    font=("Anton", 8, "bold"), command=cardview
)
viewcard.grid(row=0, column=3, padx=14, pady=10)
selectedcard = tk.Label(
    main2frame, text="SSelected card\n", font=("Anton", 8, "bold"),
    width=12, height=2
)
selectedcard.grid(row=0, column=4, padx=12, pady=10)
printcard = tk.Button(
    main2frame, text="Print card", width=10, height=2, relief='flat',
    font=("Anton", 8, "bold"), command=printbutton
)
printcard.grid(row=0, column=5, padx=12, pady=10)

addframe = tk.Frame(root, bg="#1f57a2", width=670, height=350)
title = tk.Label(
    addframe, text="Monster Card Catalogue", bg="#1f57a2",
    font=("Anton", 30, "bold")
)
title.pack()
...
namelabel = tk.Label(
    addframe, bg="#1f57a2", font=("Anton", 24, "bold"), text="Name:"
)
namelabel.place(x=270, y=60)
strengthlabel = tk.Label(
    addframe, bg="#1f57a2", font=("Anton", 24, "bold"), text="Strength:"
)
strengthlabel.place(x=270, y=100)
speedlabel = tk.Label(
    addframe, bg="#1f57a2", font=("Anton", 24, "bold"), text="Speed:"
)
speedlabel.place(x=270, y=140)
stealthlabel = tk.Label(
    addframe, bg="#1f57a2", font=("Anton", 24, "bold"), text="Stealth:"
)
stealthlabel.place(x=270, y=180)
cunninglabel = tk.Label(
    addframe, bg="#1f57a2", font=("Anton", 24, "bold"), text="Cunning:"
)
cunninglabel.place(x=270, y=220)

nameinput = tk.Entry(addframe, width=10, font=("Anton", 20, "bold"))
nameinput.place(x=420, y=65)
strengthinput = tk.Entry(
    addframe, width=10, font=("Anton", 20, "bold"),
...
stealthinput = tk.Entry(
    addframe, width=10, font=("Anton", 20, "bold"),
    validate="key", validatecommand=(vcmd, '%P')
)
stealthinput.place(x=420, y=185)
cunninginput = tk.Entry(
    addframe, width=10, font=("Anton", 20, "bold"),
    validate="key", validatecommand=(vcmd, '%P')
)
...
savebutton = tk.Button(
    addframe, text="preview", width=10, height=1, relief='flat',
    font=("Anton", 24, "bold"), command=preview
)
...
title = tk.Label(
    previewframe, text="Monster Card Catalogue", bg="#1f57a2",
    font=("Anton", 30, "bold")
)
title.pack()

namelabel = tk.Label(
    previewframe, bg="#1f57a2", font=("Anton", 24, "bold"), text="Name:"
)
namelabel.place(x=270, y=50)
strengthlabel = tk.Label(
    previewframe, bg="#1f57a2", font=("Anton", 24, "bold"), text="Strength:"
)
strengthlabel.place(x=270, y=90)
speedlabel = tk.Label(
    previewframe, bg="#1f57a2", font=("Anton", 24, "bold"), text="Speed:"
)
speedlabel.place(x=270, y=130)
stealthlabel = tk.Label(
    previewframe, bg="#1f57a2", font=("Anton", 24, "bold"), text="Stealth:"
)
stealthlabel.place(x=270, y=170)
cunninglabel = tk.Label(
    previewframe, bg="#1f57a2", font=("Anton", 24, "bold"), text="Cunning:"
)
cunninglabel.place(x=270, y=210)
totallabel = tk.Label(
    previewframe, bg="#1f57a2", font=("Anton", 24, "bold"),
    text="Total Stats:"
)
totallabel.place(x=270, y=250)

nameresult = tk.Label(
    previewframe, bg="#1f57a2", font=("Anton", 24, "bold"), text=""
)
nameresult.place(x=420, y=50)
strengthresult = tk.Label(
    previewframe, bg="#1f57a2", font=("Anton", 24, "bold"), text=""
)
strengthresult.place(x=420, y=90)
speedresult = tk.Label(
    previewframe, bg="#1f57a2", font=("Anton", 24, "bold"), text=""
)
speedresult.place(x=420, y=130)
stealthresult = tk.Label(
    previewframe, bg="#1f57a2", font=("Anton", 24, "bold"), text=""
)
stealthresult.place(x=420, y=170)
cunningresult = tk.Label(
    previewframe, bg="#1f57a2", font=("Anton", 24, "bold"), text=""
)
cunningresult.place(x=420, y=210)
totalresult = tk.Label(
    previewframe, bg="#1f57a2", font=("Anton", 24, "bold"), text=""
)
totalresult.place(x=450, y=250)
backbutton = tk.Button(
    previewframe, text="Back to edit", width=10, height=1, relief='flat',
    font=("Anton", 20, "bold"), command=backedit
)
backbutton.place(x=150, y=300)
savebutton = tk.Button(
    previewframe, text="Save Card", width=10, height=1, relief='flat',
    font=("Anton", 20, "bold"), command=saving
)
...
title = tk.Label(
    viewframe, text="Monster Card Catalogue", bg="#1f57a2",
    font=("Anton", 30, "bold")
)
title.pack()

namelabel = tk.Label(
    viewframe, bg="#1f57a2", font=("Anton", 24, "bold"), text="Name:"
)
namelabel.place(x=270, y=50)
strengthlabel = tk.Label(
    viewframe, bg="#1f57a2", font=("Anton", 24, "bold"), text="Strength:"
)
strengthlabel.place(x=270, y=90)
speedlabel = tk.Label(
    viewframe, bg="#1f57a2", font=("Anton", 24, "bold"), text="Speed:"
)
speedlabel.place(x=270, y=130)
stealthlabel = tk.Label(
    viewframe, bg="#1f57a2", font=("Anton", 24, "bold"), text="Stealth:"
)
stealthlabel.place(x=270, y=170)
cunninglabel = tk.Label(
    viewframe, bg="#1f57a2", font=("Anton", 24, "bold"), text="Cunning:"
)
cunninglabel.place(x=270, y=210)
totallabel = tk.Label(
    viewframe, bg="#1f57a2", font=("Anton", 24, "bold"), text="Total Stats:"
)
totallabel.place(x=270, y=250)

nameviewoutput = tk.Label(
    viewframe, bg="#1f57a2", font=("Anton", 24, "bold"), text=""
)
nameviewoutput.place(x=420, y=50)
strengthviewoutput = tk.Label(
    viewframe, bg="#1f57a2", font=("Anton", 24, "bold"), text=""
)
strengthviewoutput.place(x=420, y=90)
speedviewoutput = tk.Label(
    viewframe, bg="#1f57a2", font=("Anton", 24, "bold"), text=""
)
speedviewoutput.place(x=420, y=130)
stealthviewoutput = tk.Label(
    viewframe, bg="#1f57a2", font=("Anton", 24, "bold"), text=""
)
stealthviewoutput.place(x=420, y=170)
cunningviewoutput = tk.Label(
    viewframe, bg="#1f57a2", font=("Anton", 24, "bold"), text=""
)
cunningviewoutput.place(x=420, y=210)
totalviewoutput = tk.Label(
    viewframe, bg="#1f57a2", font=("Anton", 24, "bold"), text=""
)
totalviewoutput.place(x=450, y=250)
backbutton = tk.Button(
    viewframe, text="Back to menu", width=10, height=1, relief='flat',
    font=("Anton", 20, "bold"), command=backmenu
)
